...
async function onRequestPost2(context) {
  ...
  try {
    ...
    await env.DB.prepare(
      ...
      data.estilo_sugerido || "N\xE3o informado"
    ).run();
    return new Response(JSON.stringify({ success: true }), {
      headers: { "Content-Type": "application/json" }
    });
...
async function onRequestDelete(context) {
  ...
  if (!id) {
    return new Response("ID n\xE3o fornecido", { status: 400 });
  }
  try {
    await env.DB.prepare("DELETE FROM agendamentos WHERE id = ?").bind(id).run();
    return new Response(JSON.stringify({ success: true }), {
...
async function onRequestPost4(context) {
  ...
  try {
    ...
    if (slots.length > 0) {
      const statements = slots.map(
        (slot) => env.DB.prepare("INSERT INTO configuracao_agenda (client_slug, dia_semana, horario) VALUES (?, ?, ?)").bind(client_slug, slot.dia_semana, slot.horario)
      );
...
async function onRequestPost5(context) {
  ...
  try {
    ...
    const { client_slug, nome, nota, mensagem } = body;
    if (!nome || !nota || !mensagem) return Response.json({ success: false, error: "Dados incompletos" }, { status: 400 });
    await env.DB.prepare("INSERT INTO feedbacks (client_slug, nome, nota, mensagem) VALUES (?, ?, ?, ?)").bind(client_slug || "jeff-braga", nome, nota, mensagem).run();
...
async function onRequestDelete2(context) {
  ...
  try {
    ...
    if (!id) return Response.json({ success: false, error: "ID n\xE3o informado" }, { status: 400 });
...
// ../.wrangler/tmp/pages-Sk3XLq/functionsRoutes-0.34605412039558825.mjs
var routes = [
  {
